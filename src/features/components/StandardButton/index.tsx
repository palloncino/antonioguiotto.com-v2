...
export default function StandardButton({primaryText, secondaryText, primaryTextVariant, secondaryTextVariant, redirectURL, copyToClipBoard, handleCopyToClipboard, bg, buttonHeight}: StandardButtonProps) {
	...
	return (
		<StyledStandardButton
			...
			<TextContainerStyle isMobile={isMobile} className="StyledStandardButton__TextContainer">
				<Text variant={primaryTextVariant} style={{color: '#fff', padding: '0rem .2rem'}}>
					{redirectURL ? <Anchor color="#fff" href={`https://${redirectURL}`} text={primaryText}/> : primaryText}
				</Text>
				<Text block nowrap variant={secondaryTextVariant} style={{color: '#fff', padding: '0rem .2rem'}}>
					{secondaryText}
				</Text>
			</TextContainerStyle>
		</StyledStandardButton>
